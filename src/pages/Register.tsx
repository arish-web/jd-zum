...
export function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        company: formData.role === "photo" || formData.role === "tattoo" ? formData.company : undefined,
      };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100 px-4">
      <div className={`${isDarkMode ? "bg-gray-800" : "bg-white"} rounded-xl shadow-xl p-8 w-full max-w-md`}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className={`w-full px-4 py-2 rounded-md border ${isDarkMode ? "bg-gray-700" : "bg-gray-100"} focus:ring-2 focus:ring-blue-500`}
            />
          </div>
          ...
          <button
            type="submit"
            className="w-full py-2 px-4 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-md transition shadow"
          >
            Create Account
          </button>
        </form>
